...
VIEWER_SCRIPT_TEMPLATE = r"""const image=document.getElementById('frame');const video=document.getElementById('video');const state=document.getElementById('state');const base=location.pathname.replace(/\/$/,'');let fallback=false;const refresh=()=>{const next=new Image();next.onload=()=>{image.src=next.src;state.textContent='LIVE • READ ONLY'};next.onerror=()=>{state.textContent='WAITING FOR DISPLAY…'};next.src=base+'/frame?t='+Date.now()};const polling=()=>{if(fallback)return;fallback=true;video.pause();video.removeAttribute('src');video.hidden=true;image.hidden=false;image.onerror=()=>{refresh();setInterval(refresh,__INTERVAL__)};image.src=base+'/stream'};const join=parts=>{let size=0;for(const part of parts)size+=part.length;const value=new Uint8Array(size);let offset=0;for(const part of parts){value.set(part,offset);offset+= part.length}return value};const mse=async()=>{const mime='video/mp4; codecs="avc1.42E01E"';if(!window.MediaSource||!MediaSource.isTypeSupported(mime))throw Error('mse');const media=new MediaSource();video.src=URL.createObjectURL(media);await new Promise((resolve,reject)=>{media.addEventListener('sourceopen',resolve,{once:true});media.addEventListener('error',reject,{once:true})});const source=media.addSourceBuffer(mime);source.mode='segments';const queue=[];const append=()=>{if(source.updating||!queue.length)return;source.appendBuffer(queue.shift())};source.addEventListener('updateend',()=>{if(video.buffered.length){const end=video.buffered.end(video.buffered.length-1);if(end-video.currentTime>.6)video.currentTime=Math.max(0,end-.12);if(end>4&&video.buffered.start(0)<end-3){source.remove(video.buffered.start(0),end-3);return}}append();video.play().catch(()=>{})});const response=await fetch(base+'/video',{cache:'no-store'});if(!response.ok||!response.body)throw Error('video');const reader=response.body.getReader();let buffer=new Uint8Array(0),initial=[],fragment=[];while(true){const result=await reader.read();if(result.done)throw Error('ended');buffer=join([buffer,result.value]);while(buffer.length>=8){const view=new DataView(buffer.buffer,buffer.byteOffset,buffer.byteLength);const size=view.getUint32(0);if(size<8||buffer.length<size)break;const box=buffer.slice(0,size);buffer=buffer.slice(size);const type=String.fromCharCode(...box.slice(4,8));if(initial){initial.push(box);if(type==='moov'){queue.push(join(initial));initial=null;append()}}else{if(type==='moof')fragment=[box];else if(fragment.length)fragment.push(box);if(type==='mdat'&&fragment.length){queue.splice(1);queue.push(join(fragment));fragment=[];append()}}}}};if(__VIDEO__){video.hidden=false;image.hidden=true;video.onplaying=()=>{state.textContent='LIVE • READ ONLY'};mse().catch(polling);setTimeout(()=>{if(video.readyState===0)polling()},12000)}else polling();"""
...
def viewer_csp(interval: int, video: bool = False) -> str:
    ...
    return (
        "default-src 'none'; style-src 'unsafe-inline'; img-src 'self'; media-src 'self' blob:; connect-src 'self'; "
